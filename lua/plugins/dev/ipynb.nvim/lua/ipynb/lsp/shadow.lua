  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(args)
      ...
      local function emit_for_buf(bufnr)
        ...
        if not require('ipynb.config').get().float.edit_in_place then
          -- Mark as attached so Snacks.util.lsp.on can see it.
          -- Direct facade editing must not mark the facade attached: Neovim's
          -- change tracker would send didChange for a buffer the client never
          -- initialized.
