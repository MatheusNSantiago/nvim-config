...
---Synchronize direct edits made in the facade buffer.
---@param state NotebookState
function M.sync_facade_edit(state)
...
---Set up direct facade editing synchronization.
---@param state NotebookState
function M.setup_facade_sync(state)
  ...
  local buf = state.facade_buf
  local group = vim.api.nvim_create_augroup('NotebookFacadeSync_' .. buf, { clear = true })
    ...
    callback = function()
      vim.schedule(function()
        if vim.api.nvim_buf_is_valid(buf) then
          pcall(M.sync_facade_edit, state)
        end
      end)
    end,
  })
