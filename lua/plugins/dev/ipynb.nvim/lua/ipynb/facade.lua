...
function M.create(state, buf)
  ...
  vim.bo[buf].modifiable = require('ipynb.config').get().float.edit_in_place == true
  ...
  require('ipynb.cells').setup_facade_sync(state)
  ...
  vim.api.nvim_create_autocmd('BufWinEnter', {
    ...
    callback = function()
      ...
      vim.wo[win].concealcursor = 'nvic' -- Keep notebook markers concealed while editing in place
...
function M.refresh(state)
  ...
  vim.bo[buf].modifiable = true
  set_facade_lines(buf, 0, -1, lines)
  if require('ipynb.config').get().float.edit_in_place then
    vim.bo[buf].modifiable = true
  elseif not state.edit_state then
