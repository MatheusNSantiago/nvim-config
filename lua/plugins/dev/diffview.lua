...
-- Atualiza o painel esquerdo do diffview
local update_left_pane = function()
	pcall(function()
		local lib = require('diffview.lib')
		local view = lib.get_current_view()
		if view then
			-- Atualiza o painel com todos os arquivos, mas não atualiza os buffers
			view:update_files()
		end
	end)
end
...
U.directory_watcher.registerOnChangeHandler('diffview', function(filepath, events)
	local is_in_dot_git_dir = filepath:match('/%.git/') or filepath:match('^%.git/')

	if is_in_dot_git_dir or not is_git_ignored(filepath) then update_left_pane() end
end)
...
function M.config()
	...
	-- Atualiza diffview quando ganhar foco
	vim.api.nvim_create_autocmd('FocusGained', {
		callback = update_left_pane,
	})

	-- Fecha diffview ao sair da view
	vim.api.nvim_create_autocmd('User', {
		pattern = 'DiffviewViewLeave',
		callback = function() vim.cmd(':DiffviewClose') end,
	})
