...
function M.get_string_node(valid_nodes)
	...
	for _ = 1, max_depth do
		...
		if vim.tbl_contains(valid_nodes, node:type()) then return node end
...
---@param str	string
---@return boolean
function M.is_multiline(str) return str:match('\n') ~= nil end
...
function M.has_child_nodes(node)
	for child in node:iter_children() do
		if child:named() and child:type() ~= 'escape_sequence' then return true end
	end
