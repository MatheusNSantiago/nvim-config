...
function M.config()
	...
	utils.api.keymap('i', '<A-q>', M.toggleAutoSignature, { desc = 'toggle lsp signature' })
	...
	lsp_signature.setup({
		...
		-- toggle_key = '<A-q>', -- toggle signature on and off in insert mode,  e.g. toggle_key = '<M-x>'
		toggle_key = nil, -- toggle signature on and off in insert mode,  e.g. toggle_key = '<M-x>'
		floating_window = false, -- show hint in a floating window, set to false for virtual text only mode
		-- Faz o coiso dar update quando vai para outro parametro
		...
		keymaps = function(bufnr)
			utils.api.augroup('lsp-signature', {
				event = 'BufWinLeave',
				buffer = bufnr,
				once = true,
				command = function() M.disableAutoSignature() end,
			})
		end,
	})
end
...
M.highlights = {
	LspSignatureActiveParameter = { fg = c.secondary, bold = true, underline = true },
}
