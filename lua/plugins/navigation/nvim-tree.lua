...
function M._custom_commands()
	...
	return {
		...
		prompt_add = function()
			...
			node.explorer.clipboard.data.copy = selected:to_list()
			...
			if node.type == 'file' then
				...
				local filetype = M._get_filetype(path, contents)

				lines:extend({ relative_path .. ':', '', '```' .. filetype })
				lines:extend(contents)
				lines:push('```\n')
			end

			if node_is_selected then
				M.prompt_files[path] = nil
			else
				M.prompt_files[path] = lines:join('\n')
			end

			local final_prompt = vim.fn.join(vim.tbl_values(M.prompt_files), '\n')
