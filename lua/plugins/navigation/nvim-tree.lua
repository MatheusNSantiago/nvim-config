...
M.setup = function()
	return {
		...
		keys = {
			{ '<A-e>', function() M.focusOrToggle() end, mode = 'n', desc = 'nvim-tree: open explorer' },
		},
		init = function()
...
M.config = function()
	...
	require('nvim-tree').setup({
		...
		on_attach = function(bufnr)
			...
			keymap('h', api.filter.dotfiles.toggle, 'Toggle Dotfiles')
...
function M._open_on_startup(data)
	...
	local file = vim.fn.fnameescape(data.file)
	local is_real_file = (vim.fn.filereadable(file) == 1)
	-- buffer is a [No Name]
	local no_name = file == '' and vim.bo[data.buf].buftype == ''
	...
	if is_directory then
		...
		vim.cmd.cd(file) -- change to the directory
	end
...
function M._custom_commands()
	...
	return {
		...
		prompt_add = function()
			...
			if not M.prompt_paths then --
				M.prompt_paths = Array()
			end

			-- Cancela o timer existente se houver
			if M.prompt_add_timer then
				vim.fn.timer_stop(M.prompt_add_timer)
				M.prompt_add_timer = nil
			end

			local update_selection_render = function(new_selection)
				node.explorer.clipboard.data.copy = new_selection
				node.explorer.renderer:draw()
			end

			local root = node.explorer.absolute_path
			local selected_path = node.absolute_path:sub(#root + 2)

			-- -*- Renderizar nodo selecionado/não selecionado
			local selected = Array(node.explorer.clipboard.data.copy)
			local node_is_selected = selected:contains(node)
			if node_is_selected then
				selected:remove(node)
			else
				selected:push(node)
			end

			-- atualiza o render a partir dessa lista de nodos selecionados
			update_selection_render(selected:to_list())

			if not M.prompt_paths:contains(selected_path) then M.prompt_paths:push(selected_path) end

			-- Remove o path se o nodo for deselecionado
			if node_is_selected then M.prompt_paths:remove(selected_path) end

			local selected_paths = M.prompt_paths:to_list()
			if #selected_paths == 0 then return end

			local include_patterns = {}
			for _, p in ipairs(selected_paths) do
				if vim.fn.isdirectory(p) == 1 then
					table.insert(include_patterns, '-i ' .. vim.fn.shellescape(p .. '/**/*'))
				else
					table.insert(include_patterns, '-i ' .. vim.fn.shellescape(p))
				end
			end
			...
			if U.is_wsl() then
				...
				command = 'code2prompt %s %s %s --absolute-paths --quiet --hidden --output-file - | clip.exe'
			else
				command = 'code2prompt %s %s %s --absolute-paths --quiet --hidden --output-file - | wl-copy'
			end

			command = (command):format(root_path, exclude_args, include_args)
			...
			-- Timer para desmarcar os arquivos selecionados após 5 minutos
			local milliseconds = 5 * 60 * 1000 -- 5 minutos
			M.prompt_add_timer = vim.fn.timer_start(milliseconds, function()
				local current_selection = node.explorer.clipboard.data.copy
				if #current_selection > 0 then
					update_selection_render({})
					M.prompt_paths = Array()
				end

				M.prompt_add_timer = nil -- Limpa a referência do timer
			end)
		end,
		...
		open_on_default_application = function()
			local node = tree.get_node_under_cursor()
			if not node then return end
...
function M._get_filetype(path, contents)
	-- Hack para conseguir o filetype para um dado path
	-- cria um bufder temporário que imita o do path
	local name = vim.fn.fnamemodify(path, ':t')

	local temp_buf = vim.api.nvim_create_buf(false, true)
	vim.api.nvim_buf_set_name(temp_buf, name)
	vim.api.nvim_buf_set_text(temp_buf, 0, 0, 0, 0, contents)

	local filetype = vim.filetype.match({ buf = temp_buf })
...
function M._project_has_java()
	...
	local pom_path = found[1]

	-- calcula quantos níveis de diretórios existem entre o root e o pom.xml
	local relative_path = vim.fn.fnamemodify(pom_path, ':~:.')
	local depth = select(2, relative_path:gsub('/', ''))

	-- Heuristica: mais de 2 níveis de diretórios entre o root e o pom.xml significa que é um submódulo e eu posso mt bem ir lá e abrir o nvim no submódulo
	local is_submodule = depth > 2
