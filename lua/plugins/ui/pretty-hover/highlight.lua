...
function M.setup_colors(opts)
  ...
  local fg_dark, fg_light
  if normal ~= nil then
    fg_dark = M.is_dark(normal.foreground or '#ffffff') and normal.foreground or normal.background
    fg_light = M.is_dark(normal.foreground or '#ffffff') and normal.background or normal.foreground
  end
...
function M.apply_highlight(config, hl_data, bufnr)
  if M.hl_ns then api.nvim_buf_clear_namespace(bufnr, M.hl_ns, 0, -1) end
