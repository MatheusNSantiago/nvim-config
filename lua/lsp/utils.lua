...
function M.setup_document_symbols(client, bufnr)
	...
	if not symbols_supported then
		require('utils').warn('skipping setup for document_symbols, method not supported by ' .. client.name)
		return
	end
...
function M.get_client_capabilities(client_id)
	...
	if client_id ~= nil then
		...
		if not client then
			require('utils').warn('Unable to determine client from client_id: ' .. client_id)
			return
		end

		local enabled_caps = {}
		for capability, status in pairs(client.server_capabilities) do
			if status == true then table.insert(enabled_caps, capability) end
		end
		return enabled_caps
	end
end
