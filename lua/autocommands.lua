...
augroup('random_autocommands', {
  ...
  desc = 'Caso for WSL, taca o resultado do system clipboard para o clipboard do windows',
  event = 'TextYankPost',
  pattern = "*",
  command = function()
    if not U.is_wsl() then return end

    local system_register = vim.v.event.regname == '+'
